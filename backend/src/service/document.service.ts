...
import type { UnionMemberDocument, LoanDocument } from "@prisma/client";
...
export class DocumentService {
  ...
  static async deleteDocumentType(id: string) {
    ...
    const unionMemberDocsCount = await prisma.unionMemberDocument.count({
      where: {
        documentTypeId: id,
        deletedAt: null,
      },
    ...
    if (unionMemberDocsCount > 0 || loanDocsCount > 0) {
      throw new Error(
  ...
  static async uploadUnionMemberDocument(
    ...
    const document = await prisma.unionMemberDocument.create({
      data: {
        unionMemberId,
        documentTypeId,
        fileUrl,
  ...
  static async getUnionMemberDocuments(unionMemberId: string) {
    const unionMember = await prisma.unionMember.findUnique({
      where: { id: unionMemberId },
    });
    ...
    const documents = await prisma.unionMemberDocument.findMany({
      where: {
        unionMemberId,
        deletedAt: null,
      },
  ...
  static async getDocumentById(documentId: string) {
    const document = await prisma.unionMemberDocument.findUnique({
      where: {
        id: documentId,
        deletedAt: null,
      },
  ...
  static async deleteDocument(id: string, type: "unionMember" | "loan") {
    if (type === "unionMember") {
      const document = await prisma.unionMemberDocument.findUnique({
        where: { id },
      });
      ...
      await prisma.unionMemberDocument.update({
        where: { id },
  ...
  static async verifyDocument(
    ...
    if (type === "unionMember") {
      const document = await prisma.unionMemberDocument.update({
        where: { id },
        data: {
          verified,
          verificationNotes,
